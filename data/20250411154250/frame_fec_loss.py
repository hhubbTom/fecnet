...
for i in range(len(send_data)):
    ...
    #特殊处理fec包的情况，出于混流的原因，当前媒体包的fec可能会不连续出现    
    if(send_packet.payloadtype == "fec"):
        for j in range(len(frames)):
            if frames[j][0].timestamp == send_packet.timestamp:
                frames[j].append(
                    Packet(
                        recv_packet.seq,
                        send_packet.timestamp,
...
# 定义输出文件路径
output_path = os.path.join(script_dir,"frame_fec_loss.txt")
...
for i in frames:
    out.write(str(len(i)-fec_num(i))+' '+str(fec_num(i))+' '+str(loss_num(i)) )
    out.write('\n')